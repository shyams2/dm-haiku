...
def with_channels_error(f):
  @functools.wraps(f)
  def wrapper(*a, **k):
    old = layer_norm.ERROR_FOR_ERROR_PRONE_CHANNELS
    layer_norm.ERROR_FOR_ERROR_PRONE_CHANNELS = True
    try:
      return f(*a, **k)
    finally:
      layer_norm.ERROR_FOR_ERROR_PRONE_CHANNELS = old
  return wrapper
...
class LayerNormTest(parameterized.TestCase):
  ...
  def test_connection(self):
    ...
    normalize = (
        lambda a: layer_norm.LayerNorm(a, True, True, channel_axis=-1)(data))
  ...
  @test_utils.transform_and_run
  def test_channel_axis_not_required_for_final_axis(self):
  ...
  @test_utils.transform_and_run
  def test_error_prone_channel_axis(self):
    # NOTE: This test defends current, potentially error prone behaviour
    # (passing axis!=-1 and not passing channel_axis). It will be removed in a
  ...
  @test_utils.transform_and_run
  @with_channels_error
  def test_channel_axis_required_for_non_final_axis(self, axis):
    ln = layer_norm.LayerNorm(axis, True, True)
    x = jnp.ones([3, 4, 5, 6])
    with self.assertRaisesRegex(ValueError, "pass.*channel_axis.*in the ctor"):
      ln(x)
  ...
  @test_utils.transform_and_run
  def test_channel_axis_sets_param_shape(self, channel_axis, param_shape):
    ln = layer_norm.LayerNorm(-1, True, True, channel_axis=channel_axis)
    x = jnp.ones([3, 4, 5, 6])
    ln(x)
  ...
  @test_utils.transform_and_run
  def test_multiple_channel_axis(self, channel_axis, param_shape):
    ln = layer_norm.LayerNorm(-1, True, True, channel_axis=channel_axis)
    x = jnp.ones([3, 4, 5, 6])
    ln(x)
    self.assertEqual(ln.params_dict()["layer_norm/scale"].shape, param_shape)
