...
class LiftWithStateUpdater:
  ...
  def __del__(self):
    if not self._used:
      # Ideally we would throw an exception here (it should be an error to not
      # call `ignore_update` or `update`) however exceptions in __del__ are
      # suppressed by the interpreter.
      warnings.warn("LiftWithStateUpdater (from `lift_with_state`) must be "
                    "used, call `.update(..)` or `.ignore_update()` before it "
                    "goes out of scope.")
